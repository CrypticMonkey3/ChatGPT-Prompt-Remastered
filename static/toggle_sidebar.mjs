 ...
 * @param {Number} selector_right How much the modelDropdown and TuningDropdown will move to the right by, (should be the column gap property of the <ul> tag).
 * @param {String} opener_vis Whether the sidebar opener will collapse or be visible.
 */
function setElementAttrs(sidebar_width, closer_opacity, opener_opacity, selector_right, opener_vis) {
    ...
    document.getElementById("modelSelector").setAttribute("style", `right: ${selector_right}px;`);  // move right by the column gap amount
    document.getElementById("modelTuningSelector").setAttribute("style", `right: ${selector_right}px;`);  // move right by the column gap amount
